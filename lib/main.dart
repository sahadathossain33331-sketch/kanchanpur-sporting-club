...
class _AppControllerState extends State<AppController> {
  ...
  Future<void> _loadSession() async {
    ...
    try {
      ...
      // Auth user থাকলেই Home খুলবে না।
      // অবশ্যই profiles table-এ তার account থাকতে হবে।
      if (row == null) {
      ...
      // Database/profile যাচাই না হলে Home খুলবে না।
      await supabase.auth.signOut(scope: SignOutScope.local);
  ...
  Future<void> submit() async {
    ...
    try {
      ...
      if (signup) {
        ...
        // Login-এর পরে profiles table থেকে একই Username + Mobile Number যাচাই হবে।
        // তাই ভুয়া Username/Number দিয়ে অন্য account খোলা যাবে না।
        ...
        final profileName = row['name']?.toString().trim().toLowerCase() ?? '';

        if (profilePhone != mobile ||
            profileName != username.trim().toLowerCase()) {
          await supabase.auth.signOut(scope: SignOutScope.local);
