...
class _AppControllerState extends State<AppController> {
  ...
  Future<void> _loadSession() async {
    ...
    try {
      final snapshot = await db.collection('profiles').doc(current.uid).get();
...
class _AuthPageState extends State<AuthPage> {
  ...
  String authEmail(String mobile) {
    return '${normalizePhone(mobile)}@phone-login.local';
  }
  ...
  Future<void> submit() async {
    ...
    try {
      ...
      if (signup) {
        ...
        await created.updateDisplayName(username);
        ...
        final snapshot =
            await db.collection('profiles').doc(current.uid).get();

        if (!snapshot.exists || snapshot.data() == null) {
          await auth.signOut();
          throw Exception('এই Mobile Number-এর কোনো নিবন্ধিত account নেই।');
        }

        final row = snapshot.data()!;
        final profilePhone =
            normalizePhone(row['phone']?.toString() ?? '');
        final profileName =
            row['name']?.toString().trim().toLowerCase() ?? '';

        if (profilePhone != mobile ||
            profileName != username.toLowerCase()) {
          await auth.signOut();
          throw Exception('Username, Mobile Number অথবা Password ভুল।');
        }
  ...
  String _authError(FirebaseAuthException e) {
    switch (e.code) {
      case 'email-already-in-use':
        return 'এই Mobile Number দিয়ে আগে থেকেই account আছে।';
      case 'invalid-credential':
      case 'wrong-password':
      case 'user-not-found':
        return 'Username, Mobile Number অথবা Password ভুল।';
...
class _ApplicationPageState extends State<ApplicationPage> {
  ...
  Future<void> submit() async {
    ...
    try {
      ...
      if (image != null) {
        ...
        final path =
            'member-photos/${current.uid}/${DateTime.now().millisecondsSinceEpoch}.$ext';
      ...
      await db.collection('member_applications').add({
        'user_id': current.uid,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('সদস্য হওয়ার আবেদন'),
      ),
      body: ListView(
          ...
          Card(
            child: ListTile(
              ...
              trailing: IconButton(
                ...
                onPressed: () => _msg('নাম্বার: 01897173332'),
              ),
            ),
          ),
          const SizedBox(height: 12),
      ...
      body: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
        ...
        builder: (context, snapshot) {
          if (snapshot.hasError) {
            return Center(
              child: Text(
                'Leaderboard error: ${snapshot.error}',
              ),
            );
          }
          ...
          if (list.isEmpty) {
            return const Center(
              child: Text('Approved payment এখনো নেই।'),
            );
          }
          ...
          return ListView.builder(
            ...
            itemBuilder: (_, index) {
              ...
              final percent = (item.value / 100).floor();
              ...
              return Card(
                child: ListTile(
                  ...
                  subtitle: Text(
                    '${item.value} টাকা • $percent% অগ্রগতি',
                  ),
                  trailing: Text(
                    level,
                    style: const TextStyle(
    ...
    return ListView(
      ...
      children: [
        ...
        StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
          stream: db.collection('member_applications').snapshots(),
          builder: (context, snapshot) {
    ...
    return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        if (snapshot.hasError) {
          return Center(
            child: Text(
              'Applications error: ${snapshot.error}',
            ),
          );
        }
