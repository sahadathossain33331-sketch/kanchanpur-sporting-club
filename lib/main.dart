...
class _AppControllerState extends State<AppController> {
  ...
  Future<void> _loadSession() async {
    ...
    try {
      ...
      // Network/database যাচাই ব্যর্থ হলে নিরাপত্তার জন্য Home খুলবে না।
      await supabase.auth.signOut(scope: SignOutScope.local);
      if (mounted) {
        setState(() {
          user = null;
  ...
  Widget build(BuildContext context) {
    ...
    if (isAdmin) {
      return AdminHomePage(
        ...
        onLogout: () => supabase.auth.signOut(scope: SignOutScope.local),
      );
    }
...
class _AuthPageState extends State<AuthPage> {
  ...
  String authEmail(String mobile) => '${normalizePhone(mobile)}@phone-login.local';
  ...
  Future<void> submit() async {
    ...
    try {
      ...
      if (response.data is! Map) {
        throw Exception('Server থেকে সঠিক response পাওয়া যায়নি।');
      }
      ...
      // Signup সফল: এখানে Home খুলবে না। আগে Login করতে হবে।
      if (signup) {
        if (!mounted) return;
        setState(() => signup = false);
        _msg('Account সফলভাবে তৈরি হয়েছে। এখন Login করুন।');
        return;
      }

      // Login সফল বললেও client-side security check করব।
      // Server ভুলভাবে কোনো session দিলে profile না থাকলে Home খুলবে না।
      final rawSession = data['session'];
      if (rawSession is! Map) {
        throw Exception('Login session পাওয়া যায়নি।');
      }

      final session = Map<String, dynamic>.from(rawSession);
      final refreshToken = session['refresh_token']?.toString();
      if (refreshToken == null || refreshToken.isEmpty) {
        throw Exception('Login refresh token পাওয়া যায়নি।');
      }
      ...
      if (current == null) {
        await supabase.auth.signOut(scope: SignOutScope.local);
        throw Exception('Login যাচাই করা যায়নি।');
      }

      // সবচেয়ে গুরুত্বপূর্ণ check: Auth user-এর phone/profile অবশ্যই
      // যে mobile দিয়ে login করা হয়েছে সেটির সঙ্গে মিলতে হবে।
      final row = await supabase
          .from('profiles')
          .select()
          .eq('id', current.id)
      ...
      final profilePhone = normalizePhone(row['phone']?.toString() ?? '');
      final expectedEmail = authEmail(mobile).toLowerCase();
      final actualEmail = (current.email ?? '').toLowerCase();

      if (profilePhone != mobile || actualEmail != expectedEmail) {
        await supabase.auth.signOut(scope: SignOutScope.local);
      ...
      _msg(e.message.isEmpty ? 'Mobile Number অথবা Password ভুল।' : e.message);
    } catch (e) {
